...
def KalmanFilter(mu, sigma_t, z, u, A, B, C, Q, R):
    ###YOUR CODE HERE###

    #prediction step    
    mu_t_bar = A * mu + B * u
    sigma_t_bar = A * sigma_t * A.T + R

    #correction step
    S = C * sigma_t_bar * C.T + Q
    K = sigma_t_bar * C.T * np.linalg.inv(S)
    mu_new = mu_t_bar + K * (z - C * mu_t_bar)
    sigma_new = (np.eye(sigma_t_bar.shape[0]) - K * C) * sigma_t_bar
